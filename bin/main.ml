...
type ats_prop = { name : string; value : ast_value }
...
and ast_value =
  ...
  | Object of ats_prop list

let mk_ats_prop name value = { name; value }
...
let rec parse_token_to_array tokens acc =
  ...
  | token :: rest ->
      let value, rst = parse_token_to_ats token rest in
      (parse_token_to_array rst (acc @ [ value ]))
  | [] -> ([], acc)

and parse_token_to_ats token rest =
  match token with
  | Str s -> (StringLiteral s, rest)
  | Number s -> 
    if String.contains s '.' then
      (NumberLiteral (Float.of_string s), rest)
    else
      (IntegerLiteral (int_of_string s), rest)
  ...
  | LeftBracket -> 
      ...
      (Array arr, rst)   
  | Comma -> 
    parse_token_to_ats (List.hd rest) (List.tl rest)
...
and parse_token_to_properties tokens props =
  ...
  (*field:value::rest*)
  | Str fld :: _ :: v :: xs ->
      let literal, rst = parse_token_to_ats v xs in
      parse_token_to_properties rst (props @ [ mk_ats_prop fld literal ])
...
and parse tokens =
  ...
  match tokens with
  | x :: xs -> 
    let value, _ = parse_token_to_ats  x xs in
    value
  | [] -> Object []
...
let rec print_ast node =
  match node with
  | Array values -> List.iter (fun v -> print_ast v; print_string ",") values
  | Object props ->   List.iter (fun {name = name; value = value} -> print_string (name ^ "="); print_ast value; print_string "," ) props
  | other -> print_string (ats_to_str other)
...
let () =
  let file = "./dados_aleatorios.json" in
  try

    let lines = In_channel.with_open_bin file In_channel.input_all in
    print_string "file read!";
    let tokens =
      create_tokens_start
        (lines |> String.to_seq |> List.of_seq)
    in
    print_string ("tokens = " ^ Int.to_string (List.length tokens) ^ "\n");
    print_string "tokens -------\n";
    pritn_tokens tokens;
    print_string "program -------\n";
    let node = parse tokens in
    print_ast node
  with e ->
    (*close_in_noerr ic;*)
    raise e
